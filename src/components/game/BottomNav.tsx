...
export function BottomNav({ desktop = false }: { desktop?: boolean }) {
  ...
  if (desktop)
    return (
      <nav aria-label={ko ? "게임 메뉴" : "Game navigation"} className="sticky top-24 space-y-2">
        <p className="px-4 pb-3 text-xs font-bold tracking-widest text-amber-200/70">
          {ko ? "우리의 모험" : "OUR ADVENTURE"}
        </p>
        {allTabs.map((tab) => (
  ...
  return (
    <nav
      aria-label={ko ? "게임 메뉴" : "Game navigation"}
      className="mobile-game-nav sticky bottom-0 z-30 border-t border-slate-700/60 bg-slate-950/95 px-2 pt-2 backdrop-blur lg:hidden"
    >
      <ul className="mx-auto grid max-w-xl grid-cols-5">
        {tabs.slice(0, 4).map((tab) => (
          <li key={tab.id}>{renderTab(tab, true)}</li>
        ))}
        <li>
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
              <button
                type="button"
                className={`flex min-h-12 w-full flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[11px] font-semibold ${allTabs.slice(4).some((tab) => tab.id === view) ? "bg-amber-300/10 text-amber-200" : "text-slate-300"}`}
              >
            ...
            <SheetContent
              side="bottom"
              className="mobile-menu-sheet max-h-[85dvh] overflow-y-auto rounded-t-3xl border-slate-700 bg-slate-900"
            >
              <SheetTitle>{ko ? "더 많은 모험" : "More adventures"}</SheetTitle>
              <SheetDescription>
                {ko
                  ? "대결, 상점, 소환을 여기에서 찾아보세요."
                  : "Explore the arena, shop and summoning."}
              </SheetDescription>
