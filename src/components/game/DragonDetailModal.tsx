...
export interface BondSuccessDetail {
  dragonId: number;
  expGain: number;
}
...
const ELEMENT_META: Record<Element, ElementMeta> = {
  Fire: {
    labelKey: "dragon.elementsKr.Fire",
    color: "text-rose-300",
    icon: Flame,
    strong: "Wood",
    weak: "Water",
  ...
  Water: {
    labelKey: "dragon.elementsKr.Water",
    color: "text-sky-300",
    icon: Droplets,
    strong: "Fire",
    weak: "Earth",
  ...
  Wood: {
    labelKey: "dragon.elementsKr.Wood",
  ...
  Earth: {
    labelKey: "dragon.elementsKr.Earth",
  ...
  Light: {
    labelKey: "dragon.elementsKr.Light",
  ...
  Dark: {
    labelKey: "dragon.elementsKr.Dark",
...
function defaultBuildKey(d: Dragon): BuildKey {
  ...
  return (Object.entries(scores) as [BuildKey, number][]).reduce((best, cur) =>
      ...
      {visible && (
        ...
        >
          <motion.div
            ...
            className="max-h-[calc(100dvh-1rem)] w-full max-w-lg overflow-y-auto overscroll-contain rounded-t-3xl sm:rounded-3xl border border-slate-700/70 bg-gradient-to-b from-slate-900 to-slate-950 shadow-2xl shadow-black/60"
          >
            ...
            <div key={dragon.id} className="animate-in fade-in duration-200" aria-live="polite">
              <div className="flex items-start justify-between gap-3 border-b border-slate-800 px-5 py-4">
                <div className="min-w-0">
                  <p className="text-[10px] uppercase tracking-widest text-slate-500">Dragon</p>
                  <h3
                    id="dragon-detail-title"
                    className="truncate text-xl font-bold text-slate-100"
                  >
                    {dragon.name}
                  </h3>
                  <div
                    className={`mt-1 inline-flex items-center gap-1 rounded-full bg-slate-800/80 px-2 py-0.5 text-[11px] ${meta.color}`}
                  >
                    <ElIcon className="h-3 w-3" />
                    <span className="font-semibold">
                      {i18n.t("dragon.modal2.elementSuffix2", { label: i18n.t(meta.labelKey) })}
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-4 px-5 py-4">
                ...
                <section>
                  <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">
                    {i18n.t("dragon.modal.elementMatchup")}
                  </h4>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2">
                      <p className="text-[10px] uppercase text-emerald-400/80">
                        {i18n.t("dragon.modal2.strong")}
                      </p>
                      <p
                        className={`flex items-center gap-1 text-sm font-bold ${strongMeta.color}`}
                      >
                    ...
                    <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 px-3 py-2">
                      <p className="text-[10px] uppercase text-rose-400/80">
                        {i18n.t("dragon.modal2.weak")}
                      </p>
                ...
                <section>
                  <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">
                    {i18n.t("dragon.modal.recommendedBuild")}
                  </h4>
                  ...
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {build.tags.map((tag: string) => (
                        <span
                          key={tag}
                          className="rounded-full bg-slate-800/80 px-2 py-0.5 text-[10px] font-semibold text-slate-300"
                        >
                          #{tag}
                        </span>
                      ))}
...
export function TrainingSection({ dragon: baseDragon }: { dragon: Dragon }) {
  ...
  const spend = async (stat: "atk" | "hp" | "def" | "mp") => {
    ...
    if (error) {
      ...
      toast.error(
        t("dragon.training.spendFailed", {
          msg: error.message,
          defaultValue: `Stat allocation failed: ${error.message}`,
        }),
      );
      return;
    }
    toast.success(
      t("dragon.training.spendSuccess", {
        stat: stat.toUpperCase(),
        defaultValue: `${stat.toUpperCase()} upgraded!`,
      }),
    );
    ...
    <section>
      ...
      <div className="mb-2 flex items-center justify-between rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-1.5">
        <span className="text-[11px] text-slate-400">
          {t("dragon.training.remainingPoints", { defaultValue: "Remaining stat points" })}
        </span>
        ...
        {buttons.map((b) => {
          ...
          return (
            <button
              ...
              title={
                !open
                  ? t("dragon.training.closed", {
                      defaultValue: "Training ground under construction",
                    })
                  : points < 1
    ...
    <section className="relative overflow-hidden rounded-xl border border-amber-500/30 bg-slate-900/60 px-3 py-2.5">
      <div className="mb-1 flex items-center justify-between text-[11px]">
        ...
        <span className="font-mono text-slate-400">
          EXP <span className="text-slate-200">{exp.toLocaleString()}</span> /{" "}
...
export function BondingSection({ dragon }: { dragon: Dragon }) {
  ...
  const bond = async () => {
    ...
    if (tokens < 1) {
      toast.error(t("dragon.modal.tokenLow"));
      return;
    }
    setBusy(true);
    ...
    setBusy(false);
    if (error) {
      toast.error(t("dragon.bonding.bondFailed", { msg: error.message }));
      return;
    }
    setVfx(true);
