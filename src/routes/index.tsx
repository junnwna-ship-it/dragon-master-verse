...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "아티아티 드래곤 마스터즈 — 숨겨진 마법의 세계" },
      {
        name: "description",
        content:
          "아티아티 드래곤 마스터즈에 오신 것을 환영합니다. 닉네임 하나만으로 마법사 수습생이 되어 멀린의 시험에 도전하세요.",
      },
      { property: "og:title", content: "아티아티 드래곤 마스터즈" },
      {
        property: "og:description",
        content: "숨겨진 마법의 세계로 들어갈 준비가 되었나요?",
      },
    ],
  }),
      ...
      <section className="relative z-10 border-t border-slate-800/60 bg-slate-950/40 py-8 backdrop-blur">
        <div className="mb-4 px-6 text-center">
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-purple-300">
            아이들이 그린 카드가 살아 숨쉬는 곳
          </p>
          <h2 className="mt-1 text-lg font-bold text-slate-100 sm:text-xl">드래곤 쇼케이스</h2>
        </div>
...
const nicknameSchema = z
  ...
  .max(20, "닉네임은 20자 이하만 가능해요")
  .regex(/^[\p{L}\p{N}_\- ]+$/u, "한글/영문/숫자/공백만 사용할 수 있어요");
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (upErr) {
        // 닉네임 중복 등은 친절한 메시지로
        if (upErr.code === "23505") {
          throw new Error("이미 사용 중인 닉네임이에요. 다른 이름을 골라 주세요.");
        }
        throw upErr;
      }

      toast.success(`${cleaned}님, 마법의 세계에 오신 걸 환영합니다!`);
      onSuccess();
    } catch (e) {
    ...
    >
      ...
      >
        ...
        >
          {submitting ? (
            <>
              <Loader2 className="h-4 w-4 animate-spin" /> 영혼의 인장을 새기는 중...
            </>
          ) : (
            <>
              <Sparkles className="h-4 w-4" /> 마법의 세계로 입장
            </>
          )}
        </button>
        <p className="mt-3 text-center text-[10px] text-slate-500">
          가입 즉시 멀린의 시험이 시작됩니다 · 추가 정보는 필요하지 않아요
        </p>
      </motion.div>
    </motion.div>
  );
