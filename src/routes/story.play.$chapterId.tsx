      ...
      <div className="mx-auto max-w-3xl">
        ...
        {!userId ? (
          <div className="mt-8 rounded-xl border border-amber-400/30 bg-amber-400/10 p-5">
            ...
            <Link to="/" className="mt-4 inline-block rounded-lg bg-amber-300 px-4 py-2 font-semibold text-slate-950">로그인하러 가기</Link>
          </div>
        ) : (
          <>
            <button type="button" onClick={() => setCreating(true)} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-violet-300/40 bg-violet-400/15 px-4 py-3 font-bold text-violet-100 hover:bg-violet-400/25">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {owned.map((d) => (
                <Link key={d.uuid} to="/story/play/$chapterId" params={{ chapterId }} search={{ dragon: d.id }}
                  ...
                  <div><p className="font-bold">{d.name}</p><p className="text-xs text-slate-400">Lv.{byDragon.get(d.uuid!)?.level ?? 1} · {d.element}</p><p className="mt-1 text-[11px] text-amber-200">이 드래곤과 시작하기 →</p></div>
                </Link>
              ))}
            </div>
          </>
